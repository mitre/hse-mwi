...
The table below summarizes the years and geographic levels for the data sources used. For information on how measures with data available at various geographic levels were processed, see the [Geography Methods](https://github.com/mitre/hse-mwi/raw/main/Documentation/Methods/Geography%20Methods.pdf) document. 
...
- Most data is deficit-focused, and deficit-focused data may not represent community-based perspectives and lacks a focus on assets and strengths.
...
Because data were aggregated from multiple data sources, we note that all data is not from the same year or range of years. Additionally, some measures that strongly aligned with the measure evaluation and selection criteria (see below) were included even though they could not be stratified by race, so calculations based on these measures may have lower validity for Black populations. Race stratified data were available for 14 measures, unavailable for 8 measures, and not applicable for 6 measures.

Nationally representative, geographically granular data were unavailable for a wide range of topics that impact mental wellness, such as social connectedness,  sense of belonging, access to safe, reliable, and efficient transportation, community allostatic load,  and access to culturally and linguistically appropriate care, among others. If data on these, or other topics, are available at the local level, using the MWI tool data upload feature can enhance the results of the MWI. 
...
- Incorporating creative measure constructs by including measures of assets that are not “traditionally” included in health indices (ex: third places, access to financial services, voter participation, etc.), and application of novel methodologies to represent key constructs (ex: using data about treatment facility characteristics in addition to location data). 
...
Analytic and face validity verification is in progress to evaluate the robustness, validity, and accuracy of the MWI. As part of the analytic validity testing, the team will examine the correlation of the MWI with other major health indices, such as the Social Vulnerability Index (SVI), County Health Rankings (CHR), HRSA’s Service Area Status (SAS), and others.  To test its face validity, listening sessions will be held with community key informants and others to solicit feedback on ways to improve the MWI and the MWI tool to increase its validity and usefulness.  The MWI team will update this page with results as they become available.

## Downloadable Data and Information

- [Data Information](https://github.com/mitre/hse-mwi/blob/main/Data/Cleaned/HSE_MWI_Data_Information.csv): The document delineates all measures and key facets for each measure such as the data source, weight, year(s) extracted, geographic level, and directionality.
- [Mental Wellness Index for Black Americans](https://github.com/mitre/hse-mwi/raw/main/Data/Cleaned/HSE_MWI_ZCTA_Mental_Wellness_Index_Black.csv): The calculated MWI for Black Americans for each ZCTA.
- [Mental Wellness Index for American Population](https://github.com/mitre/hse-mwi/raw/main/Data/Cleaned/HSE_MWI_ZCTA_Mental_Wellness_Index_Population.csv): The calculated MWI for all Americans for each ZCTA.
- [MWI Project Github Page](https://github.com/mitre/hse-mwi): This page is home to all of the raw data, code to manipulate the data, documentation concerning each measures, and everything else concerned with the creation of the MWI Tool.
